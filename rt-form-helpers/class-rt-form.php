 ...
 *           "value"=>"myval",
 *           "class"=> array("myclass"),
 *			 "misc" => array("placeholder"=>"Enter your text"),
 *   ))."\n";
 *
 *
 *   ----textarea test
 ...
 *                   array("op1"=>1, "title"=>"title1", "checked"=>false),
 *                   array("op2"=>2, "title"=>"title2", "checked"=>true),
 *                   array("op3"=>3, "title"=>"title3", "checked"=>false),
 *           )
 *   ))."\n";
 *
 *
 ...
 *           "name"=>"myname",
 *           "class"=>array("myclass"),
 *			 "misc"=>array("title" => "title1"),
 *           "rtForm_options"=>array(
 *                   array("op1"=>1, "selected"=>false),
 *                   array("op2"=>2, "selected"=>true),
 *                   array("op3"=>3, "selected"=>false),
 *           )
 *   ))."\n";
 *
 * @author udit
...
if ( ! class_exists( 'Rt_Form' ) ) {
	...
	class Rt_Form {
		...
		private function embedd_class( $element, $class = NULL ) {
			...
			if ( isset( $class ) ) {
				...
				if ( is_array( $class ) ) {
					...
					throw new Rt_Form_Invalid_Arguments_Exception( 'class [' . $element . ']' );
				}
			}
			$html .= '" ';
		...
		private function embedd_misc_attributes( $misc ) {
			if ( ! is_array( $misc ) ) {
				throw new Rt_Form_Invalid_Arguments_Exception( 'attributes : misc' );
			}
			$html = '';
		...
		private function process_attributes( $element, $attributes, $container = false ) {
			...
			/**
			 *  list down all the classes provided along with the default class of rt_forms.
			 *  default class of rt_forms will always be attached irrespective of the attributes provided.
			 */
			if ( ! $container ) {
		...
		private function container_enclosed_elements( $element, $attrib, $rtForm_options ) {
			...
			foreach ( $rtForm_options as $opt ) {
				...
				switch ( $element ) {
					case 'rtRadio' :
						$data = '<input type="radio" ' . $checked . ' title="'. ( ( isset( $attrib['title'] ) ) ? $attrib['title'] : '' ) . '" ';
						break;
					case 'rtCheckbox' :
						$data = '<input type="checkbox" ' . $checked . ' ' . $switch . ' title="'. ( ( isset( $attrib['title'] ) ) ? $attrib['title'] : '' ) . '" ';
						break;
					case 'rtSelect' :
		...
		private function parse_multiple_options( $element, $attributes ) {
			...
			if ( is_array( $attributes ) ) {
				if ( isset( $attributes['rtForm_options'] ) && is_array( $attributes['rtForm_options'] ) ) {
					...
					throw new Rt_Form_Invalid_Arguments_Exception( 'rtForm_options [' . $element . ']' );
				}
			} else {
				throw new Rt_Form_Invalid_Arguments_Exception( 'attributes' );
			}
		}
